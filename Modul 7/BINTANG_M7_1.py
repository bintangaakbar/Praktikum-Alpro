...
class buatsendiri():    #membuat class
    ...
    def modus(self): #membuat fungsi modus
        beras = [14000,12500,13000,10000,11000,11000,9750] #variable harga beras
        modusx = max(set(beras), key=beras.count)   #membuat variable baru
        a = beras.count(modusx) #a sama dengan beras 
        b = []      #untuk list kosong agar bisa ditambah
        c = b[::a]  
        modus = "Modus data adalah "
        moduss = []
        if len(c) == 1: #membuat if else untuk hasil dari modus
            modus += str(c[0])
        else:
            for i in c:
                moduss.append(str(i))
            modus += " & ".join(moduss)
        print("modus Harga Beras                            : Rp.",modusx) #mencetak hasil dari operasi if dan else
